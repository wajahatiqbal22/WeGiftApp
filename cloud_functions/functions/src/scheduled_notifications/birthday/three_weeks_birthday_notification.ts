...
export const threeWeeksScheduledNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            for (const followingDoc of followings.docs) {
                ...
                if (notificationSettingsDoc.exists) {
                    ...
                    const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.birthday, NotificationFrequency.threeWeeks);
                    ...
                    await sendCustomNotification(`${user.firstName}'s birthday is 21 days away! Celebrate with the perfect gift!`,following);
                    try{
                        await sendPhoneMessage(following,`${user.firstName}'s birthday is 21 days away! Celebrate with the perfect gift!`);
                      }catch(e){
