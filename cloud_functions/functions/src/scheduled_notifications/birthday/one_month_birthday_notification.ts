...
export const oneMonthScheduledNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            for (const followingDoc of followings.docs) {
                ...
                if (notificationSettingsDoc.exists) {
                    ...
                    const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.birthday, NotificationFrequency.oneMonth);
                    ...
                    await sendCustomNotification(`${user.firstName}'s birthday is a month away! Shop now for the perfect gift`,following);
                    try{
                        await sendPhoneMessage(following,`${user.firstName}'s birthday is a month away! Shop now for the perfect gift`);
                      }catch(e){
                        functions.logger.error(e);  
                      }
                    // await sendReminderNotification(user, following, "Birthday", "One month");
