...
export const oneWeekScheduledNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            for (const followingDoc of followings.docs) {
                ...
                if (notificationSettingsDoc.exists) {
                    ...
                    const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.birthday, NotificationFrequency.oneWeek);
                    ...
                    // await sendReminderNotification(user, following, "Birthday", "One Week");
                    await sendCustomNotification(`One week until ${user.firstName}'s Birthday! Celebrate by letting us help you with all your gifting needs! `,following);
                    try{
                        await sendPhoneMessage(following,`One week until ${user.firstName}'s Birthday! Celebrate by letting us help you with all your gifting needs!`);
                      }catch(e){
