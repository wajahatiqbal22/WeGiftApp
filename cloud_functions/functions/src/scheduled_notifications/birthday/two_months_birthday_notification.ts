...
import { isAllowedToSendNotification, NotificationFrequency, PauseFor, sendReminderNotification } from "../reminder_notification";

export const twoMonthsScheduledNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
    .onRun(async (context) => {
        ...
        const dateAsString=now.getDate()+"-"+(now.getMonth()+2);
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            for (const followingDoc of followings.docs) {
                ...
                if (notificationSettingsDoc.exists) {
                    ...
                    await sendReminderNotification(user, following, "Birthday", "Two months");
