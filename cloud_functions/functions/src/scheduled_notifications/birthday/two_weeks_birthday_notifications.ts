...
export const twoWeeksScheduledNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
        ...
        for (const doc of usersSnapshot.docs) {
            ...
            for (const followingDoc of followings.docs) {
                ...
                if (notificationSettingsDoc.exists) {
                    ...
                    const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.birthday, NotificationFrequency.twoWeeks);
                    ...
                    // await sendReminderNotification(user, following, "Birthday", "Three weeks");
                    await sendCustomNotification(`${user.firstName}'s birthday is just 2 weeks away! Still need help with a gift? Let us guide you to the perfect present, gift card, or card!`,following);
                    try{
                        await sendPhoneMessage(following,`${user.firstName}'s birthday is just 2 weeks away! Still need help with a gift? Let us guide you to the perfect present, gift card, or card!`);
                      }catch(e){
