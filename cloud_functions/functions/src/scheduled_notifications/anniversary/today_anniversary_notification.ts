...
export const todayAnniversaryNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
            ...
            for (const doc of usersSnapshot.docs) {
                ...
                for (const followingDoc of followings.docs) {
                    ...
                    if (notificationSettingsDoc.exists) {
                        ...
                        const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.anniversary, NotificationFrequency.sameDay);
                        ...
                        await sendCustomNotification(`It's ${user.firstName}'s anniversary! Celebrate! `,following);
                        try{
                            await sendPhoneMessage(following,`It's ${user.firstName}'s anniversary! Celebrate! `);
                          }catch(e){
