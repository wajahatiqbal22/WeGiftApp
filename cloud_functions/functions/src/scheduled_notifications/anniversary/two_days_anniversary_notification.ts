...
export const twoDaysAnniversaryNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
            ...
            for (const doc of usersSnapshot.docs) {
                ...
                for (const followingDoc of followings.docs) {
                    ...
                    if (notificationSettingsDoc.exists) {
                        ...
                        const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.anniversary, NotificationFrequency.twoDay);
                        ...
                        await sendCustomNotification(`The big day is two days out! This is a final reminder to make ${user.firstName}'s anniversary the best one yet, with help from WeGift!`,following);
                        try{
                            await sendPhoneMessage(following,`The big day is two days out! This is a final reminder to make ${user.firstName}'s anniversary the best one yet, with help from WeGift!`);
                          }catch(e){
                            functions.logger.error(e);  
                          }
                        // await sendReminderNotification(user, following, "Anniversary", "One Day");
