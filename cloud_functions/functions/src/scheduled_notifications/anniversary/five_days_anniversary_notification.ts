...
export const fiveDaysAnniversaryNotifications = functions.runWith({
    "memory": "8GB",
    "timeoutSeconds": 540
}).pubsub
    .schedule("0 0 * * *")
            ...
            for (const doc of usersSnapshot.docs) {
                ...
                for (const followingDoc of followings.docs) {
                    ...
                    if (notificationSettingsDoc.exists) {
                        ...
                        const isAllowed = isAllowedToSendNotification(user, notificationSettings, PauseFor.anniversary, NotificationFrequency.fiveDays);
                        ...
                        await sendCustomNotification(`Celebrate by using Wegift to find the perfect anniversary present! `,following);
                        try{
                            await sendPhoneMessage(following,`Celebrate by using Wegift to find the perfect anniversary present! `);
                          }catch(e){
